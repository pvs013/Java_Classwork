...
public class Simulation
{
    ...
    */
    ...
    {
        // loop through people trying to add to cars
        boolean found = false;
        int p = 0;    // start of people count
        ...
        while (p < people.size())
        {
           ...
           while (!found && c < cars.size())
           {
               if ( cars.get(c).tryToAdd(people.get(p)) )
               {
                   // added the person as a passenger,  remove them
                   people.remove(p);
                   found = true;
               }
               else
