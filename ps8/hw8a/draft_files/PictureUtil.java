...
public class PictureUtil
{
    ...
    public static Picture grayAndFlipLeftToRight( Picture pic)
    {
        ...
        int[][] pix = pic.getGrayLevels();
        
        int nrows = pix.length;
        int ncols = pix[0].length;
        
        // TODO flip it left to right
        // loop by columns, up to half way
        for (int i = 0; i < ncols/2; i++)
        {
            ...
            {
                // swap the values
                int temp = pix[j][i];  // save off 
                pix[j][i] = pix[j][ncols-1-i];
                pix[j][ncols-1-i] = temp;
            }
        }
    ...
    public static Picture grayAndRotate90( Picture pic)
    {
        ...
        int irows = pix.length;   // initial picture rows
        int icols = pix[0].length;  // initial picture columns
        // TODO make a new array where the first row of the original becomes the last
        // column of the new array
        int[][] rotate = new int[icols][irows];
        
        // Loop through the picture  (i - irows, j - icols)
        for (int i = 0; i < irows; i++)
        {
            
            for (int j = 0; j < icols; j++)
            {
                rotate[j][irows-1-i] = pix[i][j];
